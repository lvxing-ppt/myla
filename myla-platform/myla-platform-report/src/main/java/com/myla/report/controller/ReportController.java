package com.myla.report.controller;
...
public class ReportController {
    ...
    public R<String> generateSampleReport(@PathVariable String barcode) {
        try {
            ...
            if (path == null) {
                return R.fail(com.myla.common.core.constant.ResultCode.NOT_FOUND, "未找到该样本的检验结果");
            }
            return R.ok(path);
        } catch (IOException e) {
            log.error("Report generation failed for barcode={}", barcode, e);
            return R.fail(com.myla.common.core.constant.ResultCode.INTERNAL_ERROR, "报告生成失败: " + e.getMessage());
        }
    }
}
