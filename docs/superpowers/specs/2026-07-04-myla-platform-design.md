...
| 模块 | 范围 |
|------|------|
| 工作流规则引擎 | 可配置的规则（阳性血培养自动触发危急值、重复样本自动合并） |
| 智能串联 | 质谱鉴定结果自动推送到 VITEK 2 药敏卡片选择 |
| 危急值预警 | 短信/邮件/屏幕弹窗多渠道通知，分级分时段 |
| 药敏判定 | 基于 CLSI/EUCAST 规则引擎的 S/I/R 自动判定 |
| 更多仪器 | BacT/ALERT、BACTEC、国产仪器适配器 |
| 质控模块 | 质控菌株管理、质控结果跟踪、失控规则判定 |
| 审计追溯 | 完整的操作审计日志、数据变更记录 |
